...
- (void)insertInclude {
    ...
    if ([self _isSymbolImported:symbol]) {
        NSString *alertString = [NSString stringWithFormat:@"#include %@ declaration exists", symbol];
        [CDRSAlert flashMessage:alertString];
    } else {
        ...
        NSString *alertString = [NSString stringWithFormat:@"Added #include %@ declaration", symbol];
        [CDRSAlert flashMessage:alertString];
    }
}
...
- (NSString *)_importDeclaration:(NSString *)symbol {
    return [NSString stringWithFormat:@"#include \"%@.h\"\n", symbol];
}
